...
# Filtrar alumnos con notas menores a 11
...
# Nueva función de fitness con penalización por violación de la restricción
def calcular_fitness(cromosoma):
    asignaciones = decodificar_cromosoma(cromosoma)
    
    # Penalización por la restricción: si todos los alumnos con notas < 11 están en el mismo examen
    penalizacion = 0
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        if all(i in indices for i in alumnos_bajos):  # Verificar si todos los alumnos con notas bajas están en este examen
            penalizacion = -10  # Penalización fuerte si la restricción se viola

...
#Implementacion de la mutacion gausiana
def mutacion_gaussiana(cromosoma, sigma=0.1):
    cromosoma_mutado = cromosoma.copy()

    for i in range(39):
        idx = i * 3
        if random.random() < 0.1:  # Probabilidad de aplicar mutación
            # Aplicamos la mutación gaussiana sobre cada peso
            for j in range(3):
                cromosoma_mutado[idx + j] += random.gauss(0, sigma)

            # Normalizamos para mantener la suma = 1
            pesos = cromosoma_mutado[idx:idx+3]
            suma = sum(pesos)
            if suma > 0:
                cromosoma_mutado[idx:idx+3] = [p / suma for p in pesos]
            else:
                cromosoma_mutado[idx:idx+3] = [1/3, 1/3, 1/3]  # Evitar división por cero
